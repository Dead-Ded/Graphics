...
class Model(Model):
    def __init__(self, height=0, width=0, scale=1, offset=0):
        ...
        self.normals, self.z_buffer = list(), list()
    ...
    def render(self):
        try:
            l = (0, 0, 1)
            mat = np.array([[(0, 0, 0)] * H] * W, dtype=np.uint8)
            self.z_buffer = np.array([[np.inf] * H] * W)
            self.calc_normals()
            for k in range(len(self.polygons)):
                ...
                for i in range(square[1], square[3] + 1):
                    for j in range(square[0], square[2] + 1):
                        bar_coords = self.__bar_coords((j, i), p)
                        zt = p[0][2] * bar_coords[0] + p[1][2] * bar_coords[1] + p[2][2] * bar_coords[2]
                        if zt < self.z_buffer[i][j] and all(map(lambda x: x >= 0, bar_coords)):
                            self.z_buffer[i][j] = zt
                            mat[i][j] = colour
